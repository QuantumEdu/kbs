...
- **Skill pack import**: Detect pack vs bare format; prefix entry IDs with namespace; backward-compatible with existing JSON imports
...
- `skillvault`: CLI adds `entry history`, `entry restore`; MCP tool count 22 → 24 (`list_entry_versions`, `restore_entry_version`)
...
**Skill Pack Export**: `VaultPackExport` wraps `VaultExport` + pack metadata. `VaultPackExportService` writes a `.svpack` file (JSON with `pack` envelope key). Import detects `pack` key → strips prefix onto entry IDs → delegates to existing `ImportAll`. Import signature extended to accept optional prefix parameter without breaking existing callers.
...
|------|--------|-------------|
| `internal/db/migrations/` | New | `009_entry_versions.sql` |
...
| `cmd/skillvault/main.go` | Modified | runCLI dispatch for new commands; vaultServices wiring |
...
- `entry_versions` is append-only: drop migration 009 + rollback code, no data loss in entries table
- Pack export is a new code path; bare export unchanged
- Pack import prefix is additive; bare import unchanged

## Dependencies

- Migration 009 runs after existing migrations 001–008
