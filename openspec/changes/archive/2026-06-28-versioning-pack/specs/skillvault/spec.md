...
**Scenarios**:
- GIVEN entry E exists, WHEN migration 006 runs, THEN `entry_versions` table is created with correct schema.
...
| ID | Requirement | Strength |
|----|-------------|----------|
| REQ-CLI-02 | Required commands: `init`, `add-entry`, `search`, `get`, `save-artifact`, `get-context`, `add-project`, `list-projects`, `archive`, `add-workflow`, `render-workflow`, `session-wrap`, `export`, `import`, `run`, `setup-vectors`, `reindex-embeddings`, `compare-entries`, `entry history`, `entry restore` | MUST |

*Delta: command count 18→20. Added `entry history` and `entry restore`.*
...
| ID | Requirement | Strength |
|----|-------------|----------|
| REQ-MCP-01 | 18 MCP tools: `save_entry`, `search_entries`, `get_entry`, `save_artifact`, `get_context`, `compose_series`, `render_workflow`, `session_wrap`, `archive_entry`, `list_projects`, `search_by_tags`, `get_context_bundle`, `save_entry_ref`, `list_entry_refs`, `get_entry_graph`, `compare_entries`, `list_entry_versions`, `restore_entry_version` | MUST |

*Delta: tool count 16→18. Added `list_entry_versions` and `restore_entry_version`.*
